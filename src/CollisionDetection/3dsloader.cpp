	...
	while (ftell (l_file) < filestatus.st_size) //Loop to scan the whole file 
	{
		...
		switch (l_chunk_id)
        {
			...
			case 0x4110: 
                ...
                for (i=0; i<l_qty; i++)
                {
                    if (fread (&p_object->vertex[i].x, sizeof(float), 1, l_file)==0)
                        std::cout << "read problem\n";
 					//printf("Vertices list x: %f\n",p_object->vertex[i].x);
                    if (fread (&p_object->vertex[i].y, sizeof(float), 1, l_file)==0)
                        std::cout << "read problem\n";
 					//printf("Vertices list y: %f\n",p_object->vertex[i].y);
                    if (fread (&p_object->vertex[i].z, sizeof(float), 1, l_file)==0)
                        std::cout << "read problem\n";
			...
			case 0x4120:
               ...
               // printf("Number of polygons: %d\n",l_qty); 
                for (i=0; i<l_qty; i++)
                {
                    if (fread (&p_object->polygon[i].a, sizeof (unsigned short), 1, l_file)==0)
                        std::cout << "read problem\n";
					//printf("Polygon point a: %d\n",p_object->polygon[i].a);
                    if (fread (&p_object->polygon[i].b, sizeof (unsigned short), 1, l_file)==0)
                        std::cout << "read problem\n";
					//printf("Polygon point b: %d\n",p_object->polygon[i].b);
                    if (fread (&p_object->polygon[i].c, sizeof (unsigned short), 1, l_file)==0)
                        std::cout << "read problem\n";
			...
			case 0x4140:
				...
				for (i=0; i<l_qty; i++)
				{
                    if (fread (&p_object->mapcoord[i].u, sizeof (float), 1, l_file)==0)
                        std::cout << "read problem\n";
					//printf("Mapping list u: %f\n",p_object->mapcoord[i].u);
                    if (fread (&p_object->mapcoord[i].v, sizeof (float), 1, l_file)==0)
                        std::cout << "read problem\n";
